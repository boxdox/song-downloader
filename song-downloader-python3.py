...
html_content_of_result_page = urllib.request.urlopen(search_url)
...
def youtube2mp3(ylink): # Youtube to mp3 function, using convertmp3.io's api! Thanks!
	...
	video_info_in_json = json.loads(urllib.request.urlopen(urllib.request.Request(video_info_in_json_url, None, headers)).read())
...
final_download_url = urllib.request.urlopen(urllib.request.Request(download_link_from_json, None, headers)) # generating the download link, with headers
